...
- DCS World with the F-100D module installed.
...
4. In DCS, open **Options → Controls → F-100D** and verify the expected device columns.
5. Confirm that the numbered kneeboard pages appear in game or OpenKneeboard.

The archive writes `Config/Input/F-100D`, the applicable `Config/Input/UiLayer` profiles, and `KNEEBOARD/F-100D`.
